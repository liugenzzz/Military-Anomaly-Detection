...
### 3. golden set：改成不需要人工精修的版本

原方案是"人工精修 500–1000 条"。现实是**没有标注人力，这条就等于不存在**。所以改成三层，前两层零人工成本（`tools/make_golden.py` 已实现）：

| 层 | 来源 | 人工成本 | 覆盖什么 |
|---|---|---|---|
| **L1 自动可验证** | 计数题、grounding 题 | **零** | 答案 100% 由 bbox 标注决定，本身就是精确真值，天然是 golden |
| **L2 借来的人工标注** | CapERA / ERA / HIVAU-70k / UCA 的保留集 | **零** | 这几个数据集**本身就是人工精标的**，切一部分出来永不进训练即可 |
| **L3 二选一审核** | LLM 生成的描述/推理题 | 约 1 小时 | 人只判「合格/不合格」，不写答案。审核比创作快十倍，100 条一小时能过完 |

**L2 是关键**：你选的 8 个数据集里有 4 个本来就是人工精标的，没必要自己再标一遍。

**如果连 L3 那一小时都没有，就只跑 L1+L2**。它们覆盖不到"描述写得好不好"，但能覆盖**数量对不对、区域准不准、异常判定对不对**——这三项恰恰是最容易退化、最影响最终效果的。有 L1+L2 远好过没有。

还有一条不需要人的替代信号：**多模型交叉一致性**。让 3 个不同模型独立回答同一题，全部一致的视为可信，只把不一致的捞出来看。能把人工量从上千条压到几十条。

```bash
python tools/make_golden.py --scenes data/screened/scenes_kept.jsonl --out-dir data/golden
```

**产出的 `exclude_ids.txt` 必须带到下游**，否则 golden set 会泄漏进训练集，评测结果全是假的：

```bash
python tools/build_vqa.py --scenes ... --exclude-ids data/golden/exclude_ids.txt
python tools/llm_qa.py generate --scenes ... --exclude-ids data/golden/exclude_ids.txt
```

两个脚本都会打印排除前后的 scene 数，跑的时候扫一眼确认它真的生效了。
...
**数据集选 8 个不选 20 个（下载地址见 [07 号文档](07_download_list.md)）；质量靠五道闸 + 分层抽检 + 自动 golden set；框架上引入 FiftyOne 做筛选，其余先用自己的脚本跑通，验证有效后再考虑迁 Data-Juicer。**
