...
航拍底座层       VisDrone / DroneCrowd / DOTA / FAIR1M / xView   → 视角、尺度、负样本
  ...
  └── llm_qa.py      LLM 侧  描述(25 个侧面, 含带框描述) / 推理
...
**越界类还有一个更管用的开关**：`MOT_BOUNDARIES`。每个序列只画一条边界线时，
一整段素材里只有轨迹中点附近那几帧算越界，其余全是负样本——24201 帧只换来两千多条 QA
就是这么来的。沿运动方向把线铺开（默认 3 条），不同的帧被不同的线截住，同一段素材产出的是
**不同的**问答：越界目标不同、时机不同、方向不同。实测 1→3→5 条时，有越界事件的帧数
大致成倍增长。

这不是凭空造数据——每条线都是一条同样合理的「禁区边界」，答案仍由真实轨迹算出。
但**图还是那些图**，调太高只是在同一批画面上反复出题，别超过 5。

**加线到一定程度就没用了。** 实测每个过筛帧已经能出 2.3 条越界 QA，说明绝大多数帧
都已经被某条线截住了——产量上限是**过筛后的帧数**，不是线的条数。所以越界类的
两个真开关是：`STRIDE_MOT` 抽得更密（默认 4，比别的源密一倍），以及把 VisDrone MOT 的
`val` / `test-dev` 两个 split 一起收进来（那是**全新素材**，不是同一批画面）——
`run_all.sh` 现在三个 split 都会自动找。它们只是 VisDrone 官方的划分，
与我们自己的 train/test 切分无关。
...
要求是三件事，每一件都有对应的题型在产出它：

| 要求 | 数据里对应什么 | 谁生成 |
|---|---|---|
| 识别异常军事活动（集结/爆炸/烟雾/越界） | `judge` 判定题 + `correct` 纠错题 + `negation` 否定题 | 规则 |
| **多模态描述（文字+图像区域）** | **`grounded` 带框描述**：一段自然语言 + 该区域的 `bbox_2d` | 文字 LLM / **坐标规则** |
| 生成异常说明 | `reason` 推理题 + `evidence` 判定依据侧面 | LLM（FACTS 约束） |

**带框描述里，文字由模型写，坐标由规则算，模型一个数字都不许写**（`must-not` 挡住
「坐标/bbox/像素」这类词），落盘时管线把规则算出来的框接在描述后面：

```json
{"role": "assistant", "content":
 "画面右上那片开阔地上集中停放着一批军用飞机，沿一条斜线排开，间距大致相等。\n{\"bbox_2d\":[333,158,459,242],\"label\":\"异常聚集区域\"}"}
```

让模型自己报坐标是整套数据里最容易出错的一件事——它会把框报得「差不多」，
而差不多的框喂进训练比不给框更糟。规则算的框和 `locate_box` 用的是**同一个函数**，
所以同一张图的定位题和描述题不会给出两个不一样的框。

三成的图还会把这三件事拼成**一条多轮对话**（`judge → grounded → reason`）：

```
用户：这幅画面属于正常态势还是异常态势？
助手：画面中出现异常：异常聚集。涉及 12 辆车辆。建议持续观察。
用户：描述异常内容并定位到具体区域。
助手：画面右上那片开阔地上…… {"bbox_2d":[389,247,512,342],"label":"异常聚集区域"}
用户：为什么不认为这是正常活动？
助手：依据有三……
```

分散在三条单轮样本里，模型学到的是三件独立的事，串不起来；拼成一轮对话，
它才会在同一段语境里把结论、区域和证据对齐。只拼三成——全拼的话模型会以为
「回答必须是三段」，单问一句「有没有异常」它也要长篇大论（`CHAIN_RATIO=` 可调）。

跑完 `check_dataset.py` 会直接打出这三件事的覆盖比例，不用自己数。
