...
1. **convoy 产量≈0。** 规则已自证正确（七种对照场景全判对）。`require_any` 要
   `military-truck/military-vehicle/armored/tank`，全库只有 Mendeley 有 `tank`，
   而它每图目标数中位 1、九成分位 5，640×640 裁剪图凑不出五辆共线坦克。
   **决策题（等用户定）**：车队机动本身不必然是军用，而"共线 + 间距均匀 + 同型 + 细长"
   四条已经把正常车流排掉了 —— 要不要放开 `require_any`？
2. **ERA 的 image_id 撞车。** `era.jsonl` 里 2112 个 id 重复且内容不同
   （一条 video 一条抽帧）。合并器现在会直接退出并打印差异，**待定位是
   适配器 id 规则的 bug 还是 era.jsonl 是两次 prepare 追加出来的**。
3. **自由看图（`vlm_free_look.md`）还没成功产出过。** 它决定后面的问法设计。
4. 集结 ~8000 / 爆炸 ~8100 / 烟雾 ~12900，**都够不到 25000/类的目标**。
