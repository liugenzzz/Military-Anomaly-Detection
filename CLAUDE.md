...
| 类 | id | 状态 | 数据来源 |
|---|---|---|---|
| 异常聚集 | `massing` | ✅ 启用 | DroneCrowd / VisDrone / Mendeley / ERA（**不含航空器**，见下） |
...
1. **convoy 与 massing 在真实数据上有大量误报 —— 自由看图证实的。**
   原始报告存档在 `docs/15_free_look_findings_raw.md`（57 张图，模型无约束描述）。
   - `smoke` 12/12、`explosion` 12/12 模型都确认 → **标签迁移类可靠**
...
- ~~convoy 产量≈0~~ → v0.5.2 放开 `require_any`（2026-09 用户决定）。
  民用车队现在也计入，"军用/民用"改由 `evidence.military_grade` 记录并进 FACTS，
  不在事件层面一刀切。排除正常车流仍靠共线 + 间距均匀 + 同型 + 细长四条。
- ~~ERA image_id 撞车~~ → 不是 video/抽帧撞车，是 `Test/` 与 `Tra/` 两个数据划分下
  有同名文件（`Baseball_001 .mp4`），而 id 里没带划分名。已在 id 中加入划分，
  并在 `dump_scenes` 加了**全局唯一性硬检查**（所有适配器共用一个写盘出口）。
